...
exports.activeEmail = function (req, res, next) {
    ...
    userDao.getByQuery({email : email}, {}, {}, function (err, models) {
        ...
        userDao.update({email : email}, {isActive : 1}, {}, function (err) {
            ...
            return res.send('<html><head><title>激活邮箱</title></head><body><p>验证成功，3秒后自动跳转到登录界面！' +
                '<a href="/auth/login">手动跳转</a></p>' +
                '<script>setInterval(function(){location.href="/auth/login"},3000);' +
                '</script></body></html>');
        });
    });
};
...
exports.signup = function (req, res, next) {
    ...
    normal.hashPass(pass, function (err, hash_pass) {
        ...
        userDao.getByQuery({email : email}, {}, {}, function (err, models) {
            ...
            mail.sendActiveMail(email, cryptoer.md5(email + config.session_secret),
                cryptoer.aesEncrypt(email, config.session_secret));
            res.json({
                errcode : 0,
...
exports.forgotPass = function (req, res, next) {
    ...
    userDao.getByQuery({email : email}, {}, {}, function (err, _ms) {
        ...
        //发送邮件
        mail.sendResetPassMail(email, cryptoer.md5(email + config.session_secret),
            cryptoer.aesEncrypt(email, config.session_secret));
        res.json({
            errcode : 0,
...
exports.updatePass = function (req, res, next) {
    ...
    if (!oldPass || oldPass.length < 6 || !newPass || newPass.length < 6 ||
        !repPass || repPass.length < 6) {
        errObj.msg = '密码信息错误！';
...
exports.list = function (req, res, next) {
    ...
    if (req.query.search) {
        var searchQuery    = validator.
                toString(validator.escape(_s.stripTags(req.query.search))).trim(),
