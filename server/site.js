...
exports.activeEmail = function (req, res, next) {
  ...
  userDao.getByQuery({email : email}, {}, {}, function (err, models) {
    ...
    userDao.update({email : email}, {isActive : 1}, {}, function (err) {
      if (err) return next(err);
      return res.send('<html><head><title>激活邮箱</title></head><body><p>验证成功，3秒后自动跳转到登录界面！<a href="/auth/login">手动跳转</a></p><script>setInterval(function(){location.href="/auth/login"},3000);</script></body></html>');
    });
  });
};
